      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-2xl shadow-xl border border-black/10 dark:border-white/15 bg-slate-900">
                <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10 transition-transform duration-300 hover:scale-105" />
              </div>
            );
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer w-[14rem] h-[19rem] sm:w-[16rem] sm:h-[22rem] md:w-[18rem] md:h-[25rem] absolute">{image}</a>
            ) : (
              <div key={index} className="fan-card w-[14rem] h-[19rem] sm:w-[16rem] sm:h-[22rem] md:w-[18rem] md:h-[25rem] absolute">{image}</div>
            );
          })}
        </div>
      </div>
